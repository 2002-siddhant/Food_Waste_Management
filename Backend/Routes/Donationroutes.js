...
const { createDonationStatusNotifications } = require('../utils/notifications.js');
...
router.post('/adddonation', authMiddleware, async (req,res)=>{
    try{
        const donation = req.body;
        const user = req.user;
        const final_donation = {
            ...donation,
            donor: user.id,
            status: 'available',
        ...
        const response = await Donation.create(final_donation);
...
router.get('/alldonations',authMiddleware, async (req,res)=>{
    try{
        await autoRejectExpiredDonations();
        const now = new Date();
...
router.get('/requested', authMiddleware, async (req, res) => {
    try {
...
router.patch('/:id/accept', authMiddleware, async (req, res) => {
    try {
        ...
        if (new Date(donation.availableTill) <= new Date()) {
            donation.status = 'rejected';
            donation.rejectionReason = 'Expired before NGO acceptance';
            await donation.save();
...
router.patch('/delivery/:deliveryId/status', authMiddleware, async (req, res) => {
    try {
        if (req.user.role !== 'delivery') {
